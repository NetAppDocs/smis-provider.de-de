...
[role="lead"]
Es ist nicht möglich, ein NetApp Speichersystem zum NetApp SMI-S Provider hinzuzufügen, wenn dieses System HTTP oder HTTPS auf einem nicht standardmäßigen Port verwendet. Das Speichersystem kann hinzugefügt werden, indem der nicht standardmäßige Port explizit angegeben wird.
