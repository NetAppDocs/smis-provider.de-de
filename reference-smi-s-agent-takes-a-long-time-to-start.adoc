...
[role="lead"]
NetApp SMI-S Provider kann auf Windows-Systemen mit bereits verwalteten Speichersystemen bis zu 15 Minuten zum Starten benötigen, da der  `smis cimserver` Befehl wartet, bis der lokale Cache des Providers gefüllt ist. Die Verwendung des  `smis cimserver` Befehls, der empfohlenen Methode zum Starten des Providers, führt zu dieser Verzögerung, bevor der Provider verfügbar ist.
