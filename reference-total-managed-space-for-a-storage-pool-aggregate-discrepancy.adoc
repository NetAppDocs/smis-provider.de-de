...
[role="lead"]
Der von NetApp SMI-S Provider für einen Speicherpool (Volume) gemeldete gesamte verwaltete Speicherplatz kann von der Größe abweichen, die von anderen Speicherverwaltungstools wie FilerView angezeigt wird, da SMI-S Provider die WAFL und Snapshot-Reserve miteinbezieht. Diese Abweichung ist zu erwarten und erfordert keine Korrekturmaßnahme.
